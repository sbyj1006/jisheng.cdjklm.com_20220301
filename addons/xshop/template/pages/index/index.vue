<template>
	<view class="container">
		...
		<view class="mp-search-box">
			<input class="ser-input" type="text" placeholder="输入关键字搜索" @confirm="submit" />
		</view>
		<!-- #endif -->
		...
		<!--通告-->
		<notice class="notice" :list="noticeList" v-if="noticeList && noticeList.length"></notice>
		...
		<view class="hot-floor" v-for="(row, index) in data.home_recommend_products" :key="index">
			...
			<scroll-view class="floor-list" scroll-x>
				<view class="scoll-wrapper">
					<view v-for="(item, i) in row.products" :key="i" class="floor-item" @click="navToDetailPage(item)">
						<image :src="item.image[0]" mode="aspectFill"></image>
						<text class="title clamp">{{item.title}}</text>
