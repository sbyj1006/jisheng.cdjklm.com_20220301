					...
					<view v-for="(item,index) in tabItem.orderList" :key="index"
						...
						<view class="i-top b-b">
							...
							<view class="after-sale-status" v-if="item.after_sale_status_tip" @click="showTip(item)">
								...
								<uni-icons color="#fa436a" type="help"></uni-icons>
							</view>
						...
						<view class="action-box b-t">
							...
							<button class="action-btn" v-if="item.is_delivery == 1 && item.status == 1 && item.is_received == 0 && app_info && app_info.config && app_info.config.express_open == 1" @click="goShipStatus(item)">查询物流</button>
