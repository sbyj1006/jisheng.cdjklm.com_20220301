    ...
    <view class="container">  
		...
		<view class="user-section">
			...
			<view class="vip-card-box">
				...
				<view class="tit">
					<text class="iconfont iconxinbaniconshangchuan-"></text>
					Vip会员
				</view>
			</view>
		</view>
		...
		>
			...
			<view class="history-section icon">
				...
				<list-cell icon="iconyouhuiquan1" iconColor="#5fcda2" title="我的优惠券" v-if="$tools.has_addon('xshopcoupon')" @eventClick="navPlugin('xshopcoupon', '/pages/user/coupon')"></list-cell>
				...
				<list-cell icon="iconmessagecenter" iconColor="#ef393c" title="消息中心" v-if="$tools.has_addon('xshopmsg')" :red-dot="newMsgCount > 0 ? true : false" @eventClick="navTo('/pages/message/message')"></list-cell>
    ...
    export default {
        ...
        computed: {
			...mapState({
				...
				newMsgCount: state => state.messages.count
			}),
			hasLogin() {
        ...
        methods: {
			...mapActions(['logout', 'getMessageCount']),
			init(){
			...
			checkVersion() {
				if (uni.getStorageSync('xshop.is_updated') == 1) {
					uni.showToast({
						title: '当前是最新版本'
					})
					return;
				...
				plus.runtime.getProperty(plus.runtime.appid, function(widgetInfo) {
					...
					_this.$http.post('update.v2', {version: widgetInfo.version, name: widgetInfo.name, platform}).then(res => {
						var data = res.data
						if (data.update == 1 && data.type == 1) {
							plus.runtime.openURL(data.wgtUrl)
							return;
						}
						if (data.update == 1 && data.type == 0) {
							uni.showModal({
								title: '检测到更新，是否下载？',
		...
		onShow() {
			this.getViewList()
			if (this.$tools.has_addon('xshopmsg')) {
