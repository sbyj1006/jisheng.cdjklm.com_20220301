define(['jquery'], function($) {
    ...
    var Api = {
        ...
        bindEvent: function (form, success, error) {
            ...
            events.xshop_choose_article(form)
            events.code_editor(form)
        },
        events: {
                    ...
                    $(".xshop-choose-product", form).on('click', function () {
                        ...
                        parent.Fast.api.open("xshop/product/select?multiple=" + multiple + "&ids=" + ids, "选择商品", {
                            callback: function (data) {
                                ...
                                var label_id = $(button).data("label-id") ? $(button).data("label-id") : "";
                                ...
                                if (input_id && data.multiple) {
                                    ...
                                    $('#' + label_id).val(data.row.title).trigger("change");
                                }
                            }
                        });
                        return false;
                        ...
                        parent.Fast.api.open("xshop/category/select?multiple=" + multiple, "选择分类", {
                            callback: function (data) {
                                ...
                                if (input_id && data.multiple) {
                                    ...
                                    $("#" + input_id).val(data.row.id).trigger("change").trigger("validate");
                                    $("#" + label_id).val(data.row.name.replace(data.row.spacer, ''))
                                }
                            }
                        });
                        return false;
            ...
            xshop_choose_article: function (form) {
                if ($(".xshop-choose-article", form).size() > 0) {
                    $(".xshop-choose-article", form).on('click', function () {
                        var that = this;
                        var multiple = $(this).data("multiple") ? $(this).data("multiple") : false;
                        parent.Fast.api.open("xshop/article/select?multiple=" + multiple, "选择文章", {
                            callback: function (data) {
                                var button = $("#" + $(that).attr("id"));
                                var maxcount = $(button).data("maxcount");
                                var input_id = $(button).data("input-id") ? $(button).data("input-id") : "";
            ...
            code_editor: function (form) {
                if ($('.code-editor', form).size() > 0) {
                    require(['backend/xshop/libs/codemirror/lib/codemirror', 
                    'backend/xshop/libs/codemirror/mode/htmlmixed/htmlmixed'], function(CodeMirror) {
                        var els = $('.code-editor', form);
                        $.each(els, function(index, item) {
                            var data = $(item).data();
                            var editor = CodeMirror.fromTextArea(item, {
                                mode: data.mode || 'htmlmixed',//编辑器语言
                                theme: data.theme || 'dracula', //编辑器主题
                                tabSize: 4,
                                indentUnit: 4,
