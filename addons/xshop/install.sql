...
INSERT INTO `__PREFIX__config` (`name`, `group`,`title`, `type`,`value`,`content`) 
SELECT 'xshop_wx_app_appid', 'basic', '微信开发平台绑定的移动应用Appid', 'string', '', '' FROM DUAL
WHERE NOT EXISTS 
(SELECT * FROM `__PREFIX__config` WHERE name='xshop_wx_app_appid');

INSERT INTO `__PREFIX__config` (`name`, `group`,`title`, `type`,`value`,`content`) 
SELECT 'xshop_wx_app_secret', 'basic', '微信开发平台绑定的移动应用AppSecret', 'string', '', '' FROM DUAL
WHERE NOT EXISTS 
(SELECT * FROM `__PREFIX__config` WHERE name='xshop_wx_app_secret');

CREATE TABLE IF NOT EXISTS `__PREFIX__xshop_refundlog` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `__PREFIX__xshop_template` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `__PREFIX__xshop_jshook` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `name` varchar(45) DEFAULT NULL COMMENT '钩子标识',
  `description` varchar(45) DEFAULT NULL COMMENT '描述',
  `action` varchar(500) DEFAULT NULL COMMENT '动作',
  `weigth` int(5) DEFAULT '0' COMMENT '排序',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='Js钩子表';

CREATE TABLE IF NOT EXISTS `__PREFIX__xshop_upgrade_sql` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `addon` varchar(45) DEFAULT NULL COMMENT '插件标识',
  `dir` varchar(255) DEFAULT 'sql' COMMENT '文件目录',
  `name` varchar(45) DEFAULT NULL COMMENT '文件名称',
  `create_time` int(10) DEFAULT '0' COMMENT '执行时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='SQL记录表';