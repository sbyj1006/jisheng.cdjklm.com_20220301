...
class Carseries extends Backend
{
    ...
    /**
     * Article模型对象
     * @var \app\common\model\Carseries
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\common\model\Carseries;
...
//        $this->relationSearch = true;
        ...
        if ($this->request->isAjax())
        {
            ...
            $bcode=Db::name('car_brand')->where('id',$ids)->find();
...
//            dump($bcode);
            ...
            $total = $this->model
                ...
                ->where($where)
                ->where('bcode',$bcode['bcode'])
//                ->order($sort, $order)
                ->order('sfirst_letter ASC')
                ->count();
...
//                    }
                    ...
                    $newsdata=Db::name('car_series')->order('scode desc')->find();
                    $newsbcode=$newsdata['scode']+1;
                    $params['scode']=$newsbcode;

                    $brand=Db::name('car_brand')->where('id',$ids)->order('bcode desc')->find();
                    $params['bcode']=$brand['bcode'];
//                    dump($params);
