...
class Index extends Api
{
    ...
    public function getdataa(){
        ...
        $datas['sybanners']= Db::name('xshop_nav')->where(array('nav_type'=>0))->order('sort desc')->limit(6)->select();
        $datas['sybanners_x']= Db::name('xshop_nav')->where(array('id'=>2))->order('sort desc')->limit(6)->find();
        $datas['user_ban']= Db::name('xshop_nav')->where(array('id'=>3))->order('sort desc')->limit(6)->find();
        $datas['car_brand']=Db::name('car_brand')->distinct(true)->field('bfirst_letter')->order('bfirst_letter asc')->select();
        foreach ($datas['car_brand'] as $key=>$val){
            $datas['car_brand'][$key]['carlist']=Db::name('car_brand')->where('bfirst_letter',$val['bfirst_letter'])->order('sort asc')->select();
        }
        exit(json_encode(['code'=>200,
            'msg'=>'首页数据获取成功',
            'id'=>$id,'datas'=>$datas,
    ...
    public function getcar_series(){
        $bcode=input('bcode');
        if($bcode){
//            dump($bcode);
            $datas['car_series']=Db::name('car_series')->where('bcode',$bcode)->order('scode desc')->select();
            exit(json_encode(['code'=>200,
                'msg'=>'首页数据获取成功',
                'id'=>$bcode,'datas'=>$datas,
            ]));
        }else{
            $datas='';
            exit(json_encode(['code'=>200,
    ...
    public function getcar_model(){
        ...
        if($scode){
//            dump($bcode);
            $datas['car_model']=Db::name('car_model')->where('scode',$scode)->select();
            exit(json_encode(['code'=>200,
                'msg'=>'首页数据获取成功',
                'id'=>$scode,'datas'=>$datas,
            ]));
        }else{
            $datas='';
            exit(json_encode(['code'=>200,
